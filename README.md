...
Uses a combination of React a& GraphQL

Gatsby calls itself a Modern site Genrator

Static  Site Generators : Genrates static HTML pages using a combo of templates , components & data

Stactic Site Generator (Gatsby)

-> Styatic pages are compiled at built-time befor deployment

-> Gatsby sites pages are made using React components

-> Static pages are than deployed to the web

->> Intial request  it return back content rich html which makes seo better

-> After initial request the site behahes more like on 
SPA (Single Page Application)

How to create a gatsby project

Step 1: Install Gatsby CLI : `npm install -g gatsby-cli`

Step 2. Gatsby Hello World library  :
...
Step 3. Change Directory to the foler `cd my-gatsby-site`

Step 4. Start the local development server with `npm run develop`  or `gatsby develop`

Gatsby will start a hot-reloading development environment accessible by default at http://localhost:8000

Routes are folder structure to acess webpages / as nested 

React boilerplate = `rfc`

GraphQL

-> A query language used to query data

-> Alertaive to using a REST API

-> GrapghQL API uses plugin to access data from differnt databsese it centralises different data sources in API

-> Then we have to uses GraphQl Query to access data
